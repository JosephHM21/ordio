...
// ── TAB ICONS ─────────────────────────────────────────────────────
const TabHome = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
const TabTag  = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
const TabInfo = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
const TabCart = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>

const TABS = [
  { id:"menu",  label:"Menú",      Icon:TabHome },
  { id:"promos",label:"Promos",    Icon:TabTag  },
  { id:"info",  label:"Info",      Icon:TabInfo },
  { id:"pedido",label:"Mi pedido", Icon:TabCart },
]
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5]" style={{fontFamily:"Inter, system-ui, sans-serif"}}>
      ...
      <div className="md:grid md:items-start px-3 md:px-5 py-4 pb-36 md:pb-10"
        ...
        {/* PRODUCTOS */}
        <main className={tab==="info"?"hidden md:block":""}>
      ...
      {/* ── INFO TAB MÓVIL ── */}
      {cartOpen===false && (
        <div className={`md:hidden ${tab==="info"?"block":"hidden"}`}>
          <div className="px-4 py-6 space-y-4">
            <h2 className="text-lg font-black text-gray-900 m-0">{restaurante.nombre}</h2>
            <div className="bg-white rounded-2xl p-4 space-y-4 border border-gray-100">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-semibold m-0 mb-0.5">Horario</p>
                  <p className="text-sm font-bold text-gray-900 m-0">{horario}</p>
                  <p className={`text-xs font-semibold m-0 mt-0.5 ${abierto?"text-green-500":"text-red-500"}`}>{abierto?"Abierto ahora":"Cerrado"}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-semibold m-0 mb-0.5">Dirección</p>
                  <p className="text-sm font-bold text-gray-900 m-0">{dir}</p>
                </div>
              </div>
              <a href={`https://wa.me/52${restaurante.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 no-underline">
                <div className="w-9 h-9 rounded-xl bg-green-50 flex items-center justify-center flex-shrink-0">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-semibold m-0 mb-0.5">WhatsApp</p>
                  <p className="text-sm font-bold text-green-600 m-0">{restaurante.whatsapp}</p>
                </div>
              </a>
              {insta&&(
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-xl bg-pink-50 flex items-center justify-center flex-shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#E1306C" strokeWidth="2" strokeLinecap="round"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 font-semibold m-0 mb-0.5">Instagram</p>
                    <p className="text-sm font-bold text-gray-900 m-0">{insta}</p>
                  </div>
                </div>
              )}
            </div>
            <p className="text-xs text-center text-gray-300 m-0">Impulsado por <span className="text-orange-400 font-bold">Ordio</span></p>
          </div>
        </div>
      )}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-14 bg-white border-t border-gray-100 z-50 flex items-stretch">
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>{
            setTab(t.id)
            if(t.id==="pedido") setCartOpen(true)
          }} className="flex-1 flex flex-col items-center justify-center gap-0.5 border-none cursor-pointer bg-transparent">
            <span style={{color:tab===t.id?"#111":"#bbb",display:"flex"}}><t.Icon/></span>
            <span style={{fontSize:9,fontWeight:tab===t.id?700:500,color:tab===t.id?"#111":"#aaa"}}>{t.label}</span>
          </button>
        ))}
      </div>
